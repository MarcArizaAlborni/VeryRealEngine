...
Component_Source::Component_Source(Game_Object* ComponentOwner, bool is_Spatial) :Component(ComponentOwner)
{
	...
	if (is_Spatial) {
		...
		App->scene->ROOT_SCENE_OBJECT->Children_List[App->scene->ROOT_SCENE_OBJECT->Children_List.size() - 2]->ChangeParentFromObject(App->scene->ROOT_SCENE_OBJECT->Children_List[App->scene->ROOT_SCENE_OBJECT->Children_List.size() - 1]->Children_List[0]);
